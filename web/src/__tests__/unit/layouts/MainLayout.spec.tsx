...
describe('MainLayout - WebSocket 实时未读数更新 (BUG-001)', () => {
  const mockSocket = {
    on: vi.fn(),
    disconnect: vi.fn(),
  }
  ...
  describe('SPEC: BUG-001 - WebSocket 实时未读数更新', () => {
    it('应该建立 WebSocket 连接并监听 unreadCountUpdated 事件', async () => {
      // 由于 MainLayout 是一个组件，我们测试其核心逻辑
      // 模拟 useEffect 中的 WebSocket 连接逻辑
    ...
    it('应该在收到 unreadCountUpdated 事件时更新未读数', async () => {
      ...
      mockSocket.on.mockImplementation((event: string, callback: (...args: unknown[]) => void) => {
        if (event === 'unreadCountUpdated') {
          // 模拟收到 WebSocket 事件
          act(() => {
            callback({ unreadCount: 5, hasUnread: true })
          })
        }
      })
    ...
    it('应该初始化时获取未读数', async () => {
      ;(request.get as any).mockResolvedValue({ count: 3 })
